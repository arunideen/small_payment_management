...
flowchart TD
    ...
    G --> H[["Dynamic approval<br/>(see §7.4)"]]
...
flowchart TD
    ...
    N -- "Request Info (§7.5)" --> RFI["Pause SLA clock<br/>ask requester / earlier approver"]
    RFI -- "answer received" --> N
    N -- "Delegate (§7.6)" --> DG["Reassign to delegate<br/>(both identities logged)"]
    DG --> N
    N -- "SLA timeout" --> ES["Reminder 50% &rarr; escalate 100%<br/>to approver's manager"]
    ES --> N

    ADM["Admin: add / remove approver (§7.7)<br/>(reason, guardrails, audit log)"] -. "live amendment" .-> T
```
